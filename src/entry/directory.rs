...
use crate::{
    ReadBytesExt, entry::Condition, string::PascalString, version::InnoVersion,
...
impl Directory {
        ...
        R: io::Read,
    {
        ...
        let mut directory = Self {
            name: src.read_decoded_pascal_string(codepage)?,
            ..Self::default()
        };

        Condition::read(&mut src, codepage, version)?;

        if ((4, 0, 11)..(4, 1, 0)).contains(&version) {
            directory.permissions = src.read_pascal_string()?;
        }
    ...
    #[must_use]
    #[inline]
    pub fn permissions(&self) -> Option<&PascalString> {
