...
bitflags! {
    ...
    pub struct HeaderFlags: u128 {
        ...
        const UNINSTALLABLE = 1 << 114;
        const DISABLE_DIR_PAGE = 1 << 115;
        const DISABLE_PROGRAM_GROUP_PAGE = 1 << 116;
        const DISABLE_APPEND_DIR = 1 << 117;
        const ADMIN_PRIVILEGES_REQUIRED = 1 << 118;
        const ALWAYS_CREATE_UNINSTALL_ICON = 1 << 119;
        const CREATE_UNINSTALL_REG_KEY = 1 << 120;
        const BZIP_USED = 1 << 121;
        const SHOW_LANGUAGE_DIALOG = 1 << 122;
        const DETECT_LANGUAGE_USING_LOCALE = 1 << 123;
        const DISABLE_DIR_EXISTS_WARNING = 1 << 124;
        const BACK_SOLID = 1 << 125;
        const OVERWRITE_UNINSTALL_REG_ENTRIES = 1 << 126;
        const SHOW_UNDISPLAYABLE_LANGUAGES = 1 << 127;
    }
}
